...
- `cleanup`: inspect, propose actions, confirm Codex is closed, create backup or manifest, execute confirmed moves only, then verify.
- `handoff`: identify useful long sessions, create handoff docs and starter prompts, then ask before archiving.
- `automation`: design the repeatable maintenance contract only; do not schedule anything until the user confirms.
