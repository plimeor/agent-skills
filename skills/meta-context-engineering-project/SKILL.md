...
参考 OpenAI GPT-5.5 prompt guidance 时，优先把项目 prompt 改成结果优先，而不是过程优先：

- 先写目标结果、成功标准、约束和停止条件，再决定是否需要步骤
- 保留真正影响行为的规则；删掉与模型默认行为重复、只是在加强语气的句子
- `必须`、`不要`、`只` 这类绝对词只用于安全、权限、精确输出格式、工具语法或不可逆动作
- 对检索、验证和文件修改写清“何时足够”，避免无限补上下文或无授权扩范围
