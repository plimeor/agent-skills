...
- [agent-docs](skills/agent-docs/SKILL.md): Manage structured AI-collaboration docs in `.agentdocs/` plus `DECISIONS.xml`: versioned requirements, plans, tasking, decisions, and the cursor. There is no separate active-doc index; front matter owns lifecycle/discovery, the cursor owns current execution links, and Decisions live as structured XML records in `DECISIONS.xml`, not prose notes or patch notes; Decision child elements are optional and extensible by project need, with authority fields written as stable current rules. Material Requirement changes create superseding records with replaced records stamped. Promotes durable product/architecture/contract residue before deletion; maintenance actions do not become Decisions, same-question Decisions merge directly, and user-identified mistaken Decisions are removed directly. Owns document lifecycle, not plan or task-graph content quality.
