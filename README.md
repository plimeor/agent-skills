...
- [agent-role-mining](skills/agent-role-mining/SKILL.md): Reverse-engineer implicit roles and decision personas from local Claude Code / Grok session history through a reproducible five-stage pipeline (discover → clean/filter → signal extraction → role inference → replay validation) built on zero-dependency Bun scripts; produces a dual-layer roles.md with mandatory residual accounting, a delegation census over excluded low-friction sessions, an open-schema control against extraction bias, and forward-replay metrics for multi-agent design.
