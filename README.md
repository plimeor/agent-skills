...
- [agent-role-mining](skills/agent-role-mining/SKILL.md): Reverse-engineer implicit roles and decision personas from local Claude Code / Grok session history through a reproducible pipeline (discover → clean/filter → signal extraction → role inference → boundary validation) on zero-dependency Bun scripts; produces a portable dual-layer roles catalog (role list, triggers, absorb/escalate boundaries, deployment menu) with residual accounting, a delegation census, an open-schema control against extraction bias, and harness-agnostic boundary checks — multi-agent runtime is optional and user-owned.
